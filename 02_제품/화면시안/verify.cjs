...
(async()=>{
  ...
  try{
    ...
    await page.locator('[data-action="roles"]').click();assert.equal(await page.getByText('일반 사용자',{exact:true}).count(),0);await page.locator('[data-role="service"]').click();await page.locator('[data-action="enter"]').click();await page.goto(base+'#data');await page.locator('[data-action="validate"]').click();await page.getByRole('heading',{name:'예시 파일 검증 결과'}).waitFor();pass('Two role previews; source validation demo appears only for service operator');
    await context.close();
