...
작성일: 2026-09-26 · 상태: 결합 설계와 독립 로컬 UI 시안 작성, 실제 도구·모델 연결 전
...
기존 기획서의 ‘AI 도입 검토 중’은 이번 요청 이전 상태임 이번 문서는 도입 방향을 구체화하고 시안 구현 범위를 기록하지만 실제 에이전트 구축·공급자 선정·효과 검증 완료를 뜻하지 않음 기존 제출 문서와 유스케이스 원본은 수정하지 않음 화면 기준은 [DESIGN.md](../DESIGN.md)에 기록

사용자 제공 기존 화면 캡처 6개를 확인했으며 [클릭 가능한 화면 시안](../02_제품/화면시안/index.html)을 독립 HTML·CSS·JavaScript로 작성함 기존 Figma Make의 원본 코드와 연결 저장소는 여전히 미확보이며 기존 앱에 적용한 결과는 아님 전체 CSV·사업체 상품·실행 기록도 확보되지 않은 상태이므로 시안 수치와 사업체는 화면 검토용 예시로 표시함
...
| 일정과 실행 기록 | 제안에서 추가하거나 직접 작성·수정, 브라우저 로컬 저장, 입력 기록의 규칙 기반 회고 | 서버 저장, 사업체 접근 통제, 버전 충돌·저장 취소 경합 처리 |
| 역할과 자료 관리 | 사업체·서비스 운영자 역할 체험, 서비스 역할의 예시 파일 검증 화면 | 실제 로그인·서버 인증과 권한 검사, CSV 업로드·검증 처리 |

시안의 AI 초안은 실제 에이전트 실행이 아니며 근거 그래프는 온톨로지 구축 완료를 뜻하지 않음 실제 통계·LLM·서버 인증·온톨로지 추론은 미연결 상태임 시안 기본 검사 15개 항목과 회귀 검사 7개의 통과 범위는 [검증 기록](../06_증빙/화면시안/검증.md)에 기록함 위 표는 실제 서비스의 테스트 통과 기록이 아님 아래 도구·권한·검증 계약은 실제 서비스의 구현 목표로 유지
...
현재 작성 범위는 개편 설계 문서와 독립 클릭 가능 UI 시안임 사용자 제공 캡처를 바탕으로 밝은 테마·Pretendard·근거 연결·계획 도우미 체험을 구성했으며 기존 Figma 앱 수정, 실제 온톨로지 데이터 적재·추론, 모델 실행·서버 인증·서버 저장은 수행하지 않음 시안 화면·동작 검사 결과와 캡처는 검증 기록에 보존하며 실제 서비스의 평가 통과나 구현 완료로 보고하지 않음
