...
작성일: 2026-09-26 · 상태: 정본 32 UC와 화면설계 범위 대조, 실제 도구·모델 연결 전

## 적용 범위와 근거

사용자는 제주 비수기 상권 마케팅 캘린더에 온톨로지·AI 에이전트 방향의 시안을 추가하고 밝은 테마·Pretendard·조절 가능한 메뉴·로그인과 수업 기준의 화면설계서를 요청함 기준은 [기획서](../00_제출/Red_PROJECT.md)와 [3 Actor·32 UC 정본](../00_제출/Red_USECASE.md)임 F01부터 F03까지의 필수 기능, 이를 돕는 확장 검토 시연, 선택 기능 F04를 구분함

정본의 F04는 여전히 선택·검토 중임 사용자 요청으로 도입 방향과 시안을 구체화했지만 실제 구현 확정·공급자 선정·효과 검증 완료를 뜻하지 않음 가게 조건 질문·계획 초안 생성·수정 에이전트는 정본 UC에 추가되지 않은 확장 검토 범위임 실제 확대를 결정하면 PROJECT·UC·화면설계서·Figma를 함께 갱신하며 이번 문서만으로 정본 범위를 바꾸지 않음 화면 기준은 [DESIGN.md](../DESIGN.md), 제출 원고는 [Red_SCREEN.md](../00_제출/Red_SCREEN.md)에 기록

[클릭 가능한 화면 시안](../02_제품/화면시안/index.html)은 독립 HTML·CSS·JavaScript 결과임 기존 Figma Make는 참고 링크이며 이번 제출용 Figma Design 링크·프레임 캡처·공유 권한 검증은 미확보 상태임 실제 CSV·사업체 상품·실행 기록도 확보되지 않아 시안 수치와 사업체는 화면 검토용 예시로 표시함

### 현재 시안과 실제 구현의 경계

| 구분 | 시안에서 작성한 범위 | 실제 적용에 남은 범위 |
| --- | --- | --- |
| 자료와 분석 | 2025년 애월읍·구좌읍 예시 방문·소비 자료, 단위·미수집·계획 대상기간 표시 | 실제 통계 적재, 출처·이용 조건 확인, 비교 가능 여부 검증 |
| 근거 연결 | 추천·관측값·자료 버전·운영 조건·일정의 예시 관계 그래프와 관계 목록 | 도메인 식별자·스키마·관계 검증, 자료 변경 추적, 온톨로지 조회·추론 |
| AI 계획 도우미 | 목표·상품·휴무일 입력, 고정 예시 후보와 휴무일 조정, 선택 항목 적용 흐름 | LLM 호출, 도구 결과에 따른 조회·대안·중단 선택, 서버 검증과 실행 기록 |
| 일정과 실행 기록 | 직접 작성·수정·브라우저 저장 시안 필수 시간·채택·활동·클릭·예약·쿠폰·비교·요약의 화면 명세는 Red_SCREEN 기준 | 서버 저장, 사업체 접근 통제, 자료 검증, 버전 충돌·저장 취소 경합 처리 |
| 역할과 자료 관리 | 두 운영자 역할의 체험 계정 로그인과 자료 화면 일반 사용자 조회는 정본의 설계 가정으로 구분 | 실제 인증·서버 권한, 일반 조회 정책, CSV 적재·다섯 검증·기준 관리의 실제 처리 |

시안의 AI 초안은 실제 에이전트 실행이 아니며 근거 그래프는 온톨로지 구축 완료를 뜻하지 않음 실제 통계·LLM·서버 인증·온톨로지 추론은 미연결 상태임 이전 시안의 기본 검사 15개·회귀 검사 7개는 이번 기능 확장의 통과 근거가 아니며 최신 결과는 [검증 기록](../06_증빙/화면시안/검증.md)에서 확인함 위 표와 아래 도구 계약은 실제 서비스의 테스트 통과 기록이 아님

현재 화면 캡처는 사업체·서비스 운영자 역할 예시임 일반 사용자에게 UC01·UC02·UC10만 제공하는 읽기 전용 상태는 정본에 따른 설계·미검증 범위이며 비공개 일정·기록·관리 권한을 부여하지 않음
...
| 선택: 추천 이유 설명 초안 | 도입 확정 후 근거를 선택하고 설명 초안 요청 | 검증한 수치·출처를 보존해 설명 작성 | SCR-C-004 선택 영역 · UC06-1, 현재 검토 중 |
| 선택: 보고서 초안 | 도입 확정 후 기록 범위를 선택하고 초안 요청 | 확인된 기록·근거를 바탕으로 보고서 첫 작성본 구성 | SCR-B-002 선택 영역 · UC06-2, 현재 검토 중 |

필수 UC05-2 사용 결과 요약은 AI 없이 기록을 집계해 제공하며 선택 UC06-2 보고서 초안과 구분함 정해진 순서의 집계·문장 생성만 사용하는 보고서 기능은 AI 작성 워크플로로 구분함 실제 에이전트는 도구 결과와 검증 실패에 따라 다음 조회·대안·중단을 선택해야 함 고정 문구나 시간 지연 시연은 실제 에이전트 실행으로 표시하지 않음 이 구분은 [Anthropic의 공식 에이전트 설계 설명](https://www.anthropic.com/engineering/building-effective-agents)을 참고
...
1. Figma 계정 연결 후 8개 주요 화면·팝업·상태의 Design 프레임을 만들고 화면설계서의 Screen ID·Actor·UC와 일치시킴 기존 앱에 옮길 때에는 원본 코드·라우트·서버 연결을 확인
...
6. 화면설계 PDF에 Figma 캡처와 팀 공유 링크를 넣고 비로그인 열람을 확인한 뒤 제출 준비 상태를 갱신

현재 작성 범위는 정본 UC에 대응한 화면설계 원고와 독립 로컬 시안임 Figma 계정 연결·제출용 Design·공유 링크·캡처·공유 권한 확인이 남아 있으므로 제출 준비 완료로 표시하지 않음 실제 온톨로지 적재·추론, 모델 실행·서버 인증·서버 저장은 미연결이며 시안의 검사 결과를 실제 서비스 구현·평가 통과로 보고하지 않음
