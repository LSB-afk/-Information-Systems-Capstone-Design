...
| 일정과 실행 기록 | 직접 작성·수정·브라우저 저장, 소요 시간·채택·활동·클릭·예약·쿠폰 기록, 비교·필수 요약 동작 시안 | 서버 저장, 사업체 접근 통제, 자료 검증, 버전 충돌·저장 취소 경합 처리 |
| 역할과 자료 관리 | 두 운영자 역할의 체험 계정 로그인과 자료 화면 일반 사용자 조회는 정본의 설계 가정으로 구분 | 실제 인증·서버 권한, 일반 조회 정책, CSV 적재·다섯 검증·기준 관리의 실제 처리 |

시안의 AI 초안은 실제 에이전트 실행이 아니며 근거 그래프는 온톨로지 구축 완료를 뜻하지 않음 실제 통계·LLM·서버 인증·온톨로지 추론은 미연결 상태임 이번 시안의 기본 검사 15개·동작 및 회귀 검사 28개가 통과했으며 최신 결과는 [검증 기록](../06_증빙/화면시안/검증.md)에서 확인함 위 표와 아래 도구 계약은 실제 서비스의 테스트 통과 기록이 아님
